...
static unsigned int athr_qos_setmap = 0;
    ...
    switch(cmd){
        ...
        case ATHR_QOS_ETH_PORT:
            if(ethcfg->val >= 0 && ethcfg->val < 4 ) {
                athr_qos_setmap |= ATHR_PORT_PRI_EN;
                qmac->reg_rmw_clear(ethcfg->ad_name,ATHR_PRI_CTRL_PORT_0 +
                      (ethcfg->portnum * 256),~athr_qos_setmap & (ATHR_DA_PRI_EN | ATHR_TOS_PRI_EN
                       | ATHR_VLAN_PRI_EN));
        ...
        case ATHR_QOS_ETH_VLAN:
            if (ethcfg->val >= 0 && ethcfg->val < 8) {
                athr_qos_setmap |= ATHR_VLAN_PRI_EN;
                int portmap = 0;
                int i;
                ...
                for(i = 1; i < 5; i++) {
                    if((portmap & (0x1 << i)) != 0){
                        qmac->reg_rmw_clear(ethcfg->ad_name,ATHR_PRI_CTRL_PORT_0 + (i * 256),
                             ~athr_qos_setmap & (ATHR_DA_PRI_EN|ATHR_TOS_PRI_EN|ATHR_PORT_PRI_EN));
        ...
        case ATHR_QOS_ETH_IP:
            {
                if(((ethcfg->tos >> 2)>= 0&&(ethcfg->tos >> 2) < 64) && (ethcfg->val >= 0 && ethcfg->val < 4)) {
                    athr_qos_setmap |= ATHR_TOS_PRI_EN;
                    int i;
                    for(i = 1; i < 5; i++) {
                        qmac->reg_rmw_clear(ethcfg->ad_name,ATHR_PRI_CTRL_PORT_0 + (i * 256),
                                ~athr_qos_setmap & (ATHR_DA_PRI_EN|ATHR_VLAN_PRI_EN|ATHR_PORT_PRI_EN));
        ...
        case ATHR_QOS_ETH_DA:
            athr_qos_setmap |= ATHR_DA_PRI_EN;
            if (ethcfg->val >= 0 && ethcfg->val < 4) {
                int i=0;
                for(i = 1; i < 5; i++) {
                    qmac->reg_rmw_clear(ethcfg->ad_name,ATHR_PRI_CTRL_PORT_0 + (i * 256),
                       ~athr_qos_setmap & (ATHR_TOS_PRI_EN|ATHR_VLAN_PRI_EN|ATHR_PORT_PRI_EN));
                    qmac->reg_rmw_set(ethcfg->ad_name,ATHR_PRI_CTRL_PORT_0 +
