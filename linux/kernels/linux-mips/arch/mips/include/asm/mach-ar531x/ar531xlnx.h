...
#define INLINE
#else
#define DEBUG_PRINTF while (0) printk
...
extern int watchdog_start(void);     /* use internal value of margin */
extern int _watchdog_start(unsigned int milliseconds);
...
#else
#define intDisable(x) local_irq_disable()
#define intEnable(x) local_irq_enable()
#endif
