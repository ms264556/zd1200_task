...
#
#############################################################
ifdef FAKEROOT_VERSION_157
#FAKEROOT_VERSION:=1.2.10
FAKEROOT_VERSION:=1.5.7.r
else
...
$(DL_DIR)/$(FAKEROOT_SOURCE):
	( if [ ! -f $(DL_DIR)/$(FAKEROOT_SOURCE) ] ; then \
		echo "$(DL_DIR)/$(FAKEROOT_SOURCE) not found"\
	fi\
	);
...
ifdef FAKEROOT_VERSION_157
	...
	$(SED) 's,^FAKEROOT_PREFIX=.*,FAKEROOT_PREFIX=$(STAGING_DIR)/usr,g' $(STAGING_DIR)/usr/bin/fakeroot
	$(SED) 's,^FAKEROOT_BINDIR=.*,FAKEROOT_BINDIR=$(STAGING_DIR)/usr/bin,g' $(STAGING_DIR)/usr/bin/fakeroot
endif
	$(SED) 's,^PATHS=.*,PATHS=$(STAGING_DIR)/usr/lib,g' $(STAGING_DIR)/usr/bin/fakeroot
