...
char *auth_user = (char)(int)NULL;
char *auth_pass = (char)(int)NULL;
char *auth_method = (char)(int)NULL;		/* Mechanism for SMTP authentication */
char *mail_domain = (char)(int)NULL;
char *from = (char)(int)NULL;		/* Use this as the From: address */
char *hostname;
char *mailhost = "mailhub";
char *minus_f = (char)(int)NULL;
char *minus_F = (char)(int)NULL;
char *gecos;
char *prog = (char)(int)NULL;
char *root = NULL;
char *tls_cert = "/etc/ssl/certs/ssmtp.pem";	/* Default Certificate */
char *uad = (char)(int)NULL;
char *config_file = (char)(int)NULL;		/* alternate configuration file */
...
#if 1 // Ruckus change: Move it to tmp instead of home directory
#define TMP_DIR "/tmp"
	memcpy (pw->pw_dir, TMP_DIR, sizeof(TMP_DIR));
#endif
...
bool_t read_config()
{
	...
	while(fgets(buf, sizeof(buf), fp)) {
		char *begin=buf;
		char *rightside;
		int authpass = (strcasestr(buf, "AuthPass") == buf);
		/* Make comments invisible; Note "AuthPass" allows '#'. */
		if(!authpass && (p = strchr(buf, '#')) != NULL) {
			*p = (char)(int)NULL;
		}
		...
		if(p){
			rightside=begin;
			/* Note "AuthPass" allows '= ". */
			q = firsttok(&begin, authpass ? "\t\n" : "= \t\n");
		}
		if(p && q) {
			if(strcasecmp(p, "Root") == 0) {
				...
				if(log_level > 0) {
					log_event(LOG_INFO, "Set Root=\"%s\"\n", root);
		                ...
		                log_event(LOG_INFO, "ssmtp::the MailHub is %s\n",q);
		                hostport(q, ":");
			}
			else if(strcasecmp(p, "waittime") == 0) {
				log_event(LOG_DEBUG, "Set WaitTime= %s\n", q);
				wait_time = atoi(q);
				if(wait_time > 3600 || wait_time < 1){
                                       log_event(LOG_INFO, "wait_time value error:%d\n", wait_time);
                                       wait_time = 10;
				}
			}
...
#if 1 // Ruckus change: AUTH auto detect
		if(!auth_method && strstr(response, "250-AUTH")) {
			if(strstr(response, "LOGIN")) {
				auth_method = strdup("LOGIN");
				if(log_level > 0) {
					log_event(LOG_INFO, "    AUTH LOGIN auto detected\n");
				}
			} else if(strstr(response, "CRAM-MD5")) {
				auth_method = strdup("CRAM-MD5");
				if(log_level > 0) {
					log_event(LOG_INFO, "    AUTH CRAM-MD5 auto detected\n");
				}
			}
		}
#endif
...
#if 1 // Ruckus change: Time out in 10 seconds to handle unreachable server/port
        log_event(LOG_DEBUG, "the wait_time before alarm is %d\n", wait_time);
	(void)alarm(wait_time);  /* Set initial timer */
#endif
	if(setjmp(TimeoutJmpBuf) != 0) {
		/* Then the timer has gone off and we bail out */
		die("Connection lost in middle of processing");
	}
        log_event(LOG_DEBUG, "the mail host is %s, the port is %d\n", mailhost, port);
