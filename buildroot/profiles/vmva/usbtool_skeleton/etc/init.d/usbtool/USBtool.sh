...
DetectTargets () {
  for d in $_TRYTARGET; do
    cat /proc/diskstats | grep $d 1>/dev/null 2>&1
    if [ $? -eq 0 ]; then
      #is in diskstats, see if it is mounted as source
      mount | grep $d 1>/dev/null 2>&1
      if [ $? -ne 0 ]; then
        _CFCARD="${d}"
        _BOOTFS="${d}1"
        _ROOTFS="${d}2 ${d}3"
        _DATAFS="${d}4"
        echo "$_CFCARD" > /proc/rks_hw_info/rbd_dev
        return 0
      fi
    fi
  done
...
DetectTargets
if [ $? -ne 0 ]; then
  echo "Cannot find targets for installing images, abort!"
else
  echo "CF=$_CFCARD, Bootfs=$_BOOTFS, Rootfs=$_ROOTFS, Datafs=$_DATAFS"
