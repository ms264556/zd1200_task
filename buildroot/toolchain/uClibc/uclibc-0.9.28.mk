...
#
#############################################################
ifeq ($(BR2_UCLIBC_BUILD_DEBUG),y)
UCLIBC_BUILD_TYPE=debug
else
UCLIBC_BUILD_TYPE=release
endif

UCLIBC_CONFIG_FILE=$(strip $(subst ",, $(BR2_UCLIBC_CONFIG))) 
...
UCLIBC_INSTALL=$(TARGET_DIR)

ifeq ($(BR2_UCLIBC_USBTOOL),y)
UCLIBC_INSTALL+=$(USB_TOOL_BUILD_DIR)
endif
...
$(UCLIBC_DIR)/.configured: $(UCLIBC_DIR)/.unpacked  $(UCLIBC_CONFIG_FILE) $(LINUX_HEADERS_DIR)/.configured
...
ifeq ($(UCLIBC_BUILD_TYPE),debug)
	$(SED) 's,# DODEBUG is not set,DODEBUG=y,g' $(UCLIBC_DIR)/.config
	$(SED) 's,# DODEBUG_PT is not set,DODEBUG_PT=y,g' $(UCLIBC_DIR)/.config
endif
...
$(STAGING_DIR)/lib/libc.a: $(UCLIBC_DIR)/lib/libc.a $(LINUX_HEADERS_DIR)/.configured
	$(MAKE1) -C $(UCLIBC_DIR) \
		PREFIX= \
...
ifneq ($(UCLIBC_INSTALL),)
uclibc-install: $(STAGING_DIR)/lib/libc.a
	for d in $(UCLIBC_INSTALL); do \
		$(MAKE1) -C $(UCLIBC_DIR) \
			PREFIX=$$d \
			DEVEL_PREFIX=/usr/ \
			RUNTIME_PREFIX=/ \
			install_runtime; \
		touch -c $$d/lib/libc.so.0; \
	done;

uclibc-utils-install:
	for d in $(UCLIBC_INSTALL); do \
		$(MAKE1) -C $(UCLIBC_DIR) $(TARGET_CONFIGURE_OPTS) \
			PREFIX=$$d utils install_utils; \
		if [ $(BR2_CROSS_TOOLCHAIN_TARGET_UTILS)=y ]; then \
			mkdir -p $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/target_utils; \
			install -c $$d/usr/bin/ldd \
				$(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/target_utils/ldd; \
		fi; \
		touch -c $$d/usr/bin/ldd; \
	done;

#UCLIBC_USB_TARGETS=$(USB_TOOL_BUILD_DIR)/lib/libc.so.0
endif
...
uclibc-target-utils: uclibc-utils-install #$(TARGET_DIR)/usr/bin/ldd

uclibc-info:
	@echo "UCLIBC_CONFIG_FILE=$(UCLIBC_CONFIG_FILE)"
...
uclibc_target: gcc uclibc $(TARGET_DIR)/usr/lib/libc.a uclibc-target-utils #$(TARGET_DIR)/usr/bin/ldd
