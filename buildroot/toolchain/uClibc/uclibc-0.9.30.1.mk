...
$(UCLIBC_DIR)/.configured: $(LINUX_HEADERS_DIR)/.configured $(UCLIBC_DIR)/.config
	$(MAKE1) -C $(UCLIBC_DIR) \
		...
		install_headers
	$(TARGET_CROSS)gcc -nostdlib -nostartfiles -shared -x c /dev/null -o $(TOOL_BUILD_DIR)/uClibc_dev/usr/lib/libc.so
	$(TARGET_CROSS)gcc -nostdlib -nostartfiles -shared -x c /dev/null -o $(TOOL_BUILD_DIR)/uClibc_dev/usr/lib/libm.so
	cp -pLR $(UCLIBC_DIR)/lib/crt[1in].o $(TOOL_BUILD_DIR)/uClibc_dev/usr/lib/
else
...
ifeq ($(BR2_GCC_VERSION_4_3_X),y)
	cp -pLR $(TOOL_BUILD_DIR)/uClibc_dev/usr/* $(STAGING_DIR)/
...
.PHONY: uclibc-headers
uclibc-headers:
ifeq ($(LINUX_HEADERS_IS_KERNEL),y)
	cp -pLRfu $(LINUX_HEADERS_DIR)/include/* $(STAGING_DIR)/include/
else
	cp -pLRfu $(LINUX_HEADERS_DIR)/include/asm $(STAGING_DIR)/include/
	cp -pLRfu $(LINUX_HEADERS_DIR)/include/linux $(STAGING_DIR)/include/
	if [ -d $(LINUX_HEADERS_DIR)/include/asm-generic ]; then \
		cp -pLRfu $(LINUX_HEADERS_DIR)/include/asm-generic $(STAGING_DIR)/include/; \
	fi
endif
...
$(TARGET_DIR)/lib/libc.so.0: $(STAGING_DIR)/lib/libc.a
	if [ ! -f $(STAGING_DIR)/lib/.built ]; then \
		$(MAKE1) -C $(UCLIBC_DIR) \
			PREFIX=$(TARGET_DIR) \
			DEVEL_PREFIX=/usr/ \
			RUNTIME_PREFIX=/ \
			install_runtime; \
		touch $(STAGING_DIR)/lib/.built; \
	else \
		$(MAKE1) -C $(UCLIBC_DIR) \
			PREFIX=$(TARGET_DIR) \
			DEVEL_PREFIX=/usr/ \
...
ifeq ($(BR2_UCLIBC_USBTOOL),y)
UCLIBC_TARGETS+=uclibc-extracopy
endif

uclibc-extracopy: $(TARGET_DIR)/lib/libc.so.0
	$(MAKE1) -C $(UCLIBC_DIR) \
		PREFIX=$(USB_TOOL_BUILD_DIR) \
		DEVEL_PREFIX=/usr/ \
		RUNTIME_PREFIX=/ \
		install_runtime_nodep; \
	touch -c $(USB_TOOL_BUILD_DIR)/lib/libc.so.0
endif

ifeq ($(BR2_GCC_VERSION_4_3_X),y)
uclibc: $(gcc_intermediate) $(STAGING_DIR)/lib/libc.a $(UCLIBC_TARGETS) uclibc-headers
else
uclibc: $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc $(STAGING_DIR)/lib/libc.a $(UCLIBC_TARGETS) uclibc-headers
endif
...
ifeq ($(BR2_GCC_VERSION_4_3_X),y)
uclibc-test: $(STAGING_DIR)/lib/libc.a $(TARGET_DIR)/root/uClibc/test/unistd/errno
else
