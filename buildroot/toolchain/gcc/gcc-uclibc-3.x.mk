...
#only for gcc versions older than 4.3.5 (4.2.4 and older)
#yes, gcc4.2.4 is still using this file, I don't know why
ifneq ($(BR2_GCC_VERSION_4_3_X),y)
...
$(DL_DIR)/$(GCC_SOURCE):
	mkdir -p $(DL_DIR)
	( if [ ! -f $(DL_DIR)/$(GCC_SOURCE) ] ; then\
		echo "$(DL_DIR)/$(GCC_SOURCE) not found"\
	fi\
	);
...
#	if d in $(GCC_INSTALL); do
	PATH=$(TARGET_PATH) \
	$(MAKE) DESTDIR=$$d -C $(GCC_BUILD_DIR3) install
	# Remove broken specs file (cross compile flag is set).
	rm -f $$d/usr/$(GCC_LIB_SUBDIR)/specs;
#	done;
