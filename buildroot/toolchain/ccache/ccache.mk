...
$(DL_DIR)/$(CCACHE_SOURCE):
	( if [ ! -f $(DL_DIR)/$(CCACHE_SOURCE) ] ; then\
		echo "$(DL_DIR)/$(CCACHE_SOURCE) not found"\
	fi\
	);
...
$(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-gcc: $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc
	mkdir -p $(STAGING_DIR)/bin-ccache
	if [ ! -f $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-gcc ]; \
	then mv $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc $(STAGING_DIR)/bin-ccache; \
	fi 

$(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-c++: $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-c++
	mkdir -p $(STAGING_DIR)/bin-ccache
	if [ ! -f $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-c++ ]; \
	then mv $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-c++ $(STAGING_DIR)/bin-ccache; \
	fi 


$(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-g++: $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-g++
	mkdir -p $(STAGING_DIR)/bin-ccache
	if [ ! -f $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-g++ ]; \
	then mv $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-g++ $(STAGING_DIR)/bin-ccache; \
	fi 

$(STAGING_DIR)/ccache-run-always: $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-gcc $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-c++ $(STAGING_DIR)/bin-ccache/$(REAL_GNU_TARGET_NAME)-g++
	...
	(cd $(STAGING_DIR)/bin-ccache; \
		...
		ln -fs $(REAL_GNU_TARGET_NAME)-gcc $(GNU_TARGET_NAME)-gcc-4.2.4; \
	...
	(cd $(STAGING_DIR)/bin; \
		...
		ln -fs ../usr/bin/ccache $(GNU_TARGET_NAME)-cpp; \
		ln -fs ../usr/bin/ccache $(REAL_GNU_TARGET_NAME)-cc; \
		ln -fs ../usr/bin/ccache $(REAL_GNU_TARGET_NAME)-cpp; \
		ln -fs ../usr/bin/ccache $(REAL_GNU_TARGET_NAME)-gcc-4.2.4; \
...
ccache: gcc $(STAGING_DIR)/$(CCACHE_TARGET_BINARY) $(STAGING_DIR)/ccache-run-always
